...
def get_user_stats(parent_id):
    ...
    if child_ids:
        ...
        total_training_sessions = execute_db(
            f"SELECT COUNT(*) FROM training_sessions WHERE child_id IN ({placeholders})", tuple(child_ids)
        )[0][0]
        ...
        score_result = execute_db(
            f"SELECT AVG(overall_score) FROM session_summaries WHERE child_id IN ({placeholders})", tuple(child_ids)
        )
