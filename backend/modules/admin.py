...
def get_users():
    ...
    users = [
        {"id": u[0], "uid": u[1], "username": u[2], "role": u[3], "is_banned": u[4], "child_count": u[5]}
        for u in result
    ]
...
def get_admin_stats():
    ...
    return jsonify(
        {"users": users, "admins": admins, "banned": banned, "children": children, "sessions": sessions, "posts": posts}
    ), 200
...
def get_all_children():
    ...
    children = [{"id": c[0], "name": c[1], "age": c[2], "parent_name": c[3], "training_count": c[4]} for c in result]
...
def delete_user(user_id):
    ...
    execute_db(
        "DELETE FROM training_sessions WHERE child_id IN (SELECT id FROM children WHERE parent_id = ?)", (user_id,)
    )
