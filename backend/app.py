...
from modules import admin_bp, auth_bp, badges_bp, children_bp, forum_bp, knowledge_bp, user_stats_bp
from utils.error_codes import AUTH_ERROR, BAD_REQUEST, INTERNAL_ERROR, NOT_FOUND, PERMISSION_DENIED
...
def handle_unauthorized(error):
    return error_response(
        message=str(error.description) if hasattr(error, "description") else "认证失败", code=AUTH_ERROR, status=401
    )
...
def handle_not_found(error):
    if request.path.startswith("/api/"):
        return error_response(message="请求的资源不存在", code=NOT_FOUND, status=404)
    return send_from_directory(FRONTEND_DIR, "index.html")


@app.errorhandler(405)
def handle_method_not_allowed(error):
    return error_response(message="请求方法不允许", code=BAD_REQUEST, status=405)


@app.errorhandler(500)
def handle_internal_error(error):
    app.logger.error(f"Internal Server Error: {str(error)}\n{traceback.format_exc()}")
    return error_response(message="服务器内部错误", code=INTERNAL_ERROR, status=500)
