...
class ReportGenerator:
    ...
    def generate_attention_report(child_id: int, period_start: str, period_end: str) -> dict:
        ...
        child_info = {"id": child[0][0], "name": child[0][1], "age": child[0][2]}

        dimension_scores = AttentionAnalyzer.calculate_dimension_scores(child_id, period_start, period_end)
        ...
        trends = {}
        for dim in ATTENTION_DIMENSIONS:
    ...
    def generate_progress_report(child_id: int, period: str = "month") -> dict:
        ...
        if len(scores) >= 2:
            ...
            second_half_att = sum(attention_scores[len(attention_scores) // 2 :]) / (
                len(attention_scores) - len(attention_scores) // 2
            )
        ...
        for ub in user_badges:
            ...
            if badge_id in badge_dict:
                ...
                milestones.append(
                    {"name": badge["name"], "icon": badge["icon"], "earned_at": ub[1][:10] if ub[1] else None}
                )
