...
def is_request_processed(request_id):
    if not request_id:
        return False
    result = execute_db("SELECT id FROM processed_requests WHERE request_id = ?", (request_id,))
    return len(result) > 0


def mark_request_processed(request_id):
    if request_id:
        execute_db("INSERT INTO processed_requests (request_id) VALUES (?)", (request_id,))
...
def calculate_session_summary(session_id, child_id, game_type, final_score, total_accuracy, duration=None):
    ...
    session_info = execute_db("SELECT start_time FROM training_sessions WHERE id = ?", (session_id,))
    total_time = 0
...
def check_and_award_badges(child_id, summary):
    ...
    if training_count == 1:
        badge = next(
            (b for b in BADGES if b["requirement_type"] == "training_count" and b["requirement_value"] == 1), None
        )
        if badge:
            execute_db("INSERT OR IGNORE INTO user_badges (child_id, badge_id) VALUES (?, ?)", (child_id, badge["id"]))
            earned_badges.append({"id": badge["id"], "name": badge["name"], "icon": badge["icon"]})

    if training_count == 50:
        badge = next(
            (b for b in BADGES if b["requirement_type"] == "training_count" and b["requirement_value"] == 50), None
        )
        if badge:
            execute_db("INSERT OR IGNORE INTO user_badges (child_id, badge_id) VALUES (?, ?)", (child_id, badge["id"]))
            earned_badges.append({"id": badge["id"], "name": badge["name"], "icon": badge["icon"]})
    ...
    if summary.get("overall_score", 0) >= 95:
        badge = next(
            (b for b in BADGES if b["requirement_type"] == "total_score" and b["requirement_value"] == 95), None
        )
        if badge:
            execute_db("INSERT OR IGNORE INTO user_badges (child_id, badge_id) VALUES (?, ?)", (child_id, badge["id"]))
            earned_badges.append({"id": badge["id"], "name": badge["name"], "icon": badge["icon"]})
...
def generate_recommendations(summary, game_type):
    ...
    if "working_memory" in dimensions and summary.get("overall_score", 0) >= 90:
        badge = next(
            (b for b in BADGES if b["requirement_type"] == "memory_score" and b["requirement_value"] == 90), None
        )
        if badge:
    ...
    if "visual_tracking" in dimensions and summary.get("overall_score", 0) >= 90:
        badge = next(
            (b for b in BADGES if b["requirement_type"] == "tracking_score" and b["requirement_value"] == 90), None
        )
        if badge:
...
def start_session():
    ...
    result, session_id = execute_db(
        ...
        VALUES (?, ?, ?, ?, 'active')
    """,
        (child_id, game_type, session_token, device_id),
        fetch_last_id=True,
    )
    ...
    try:
        ...
        """,
            (
                ...
                session_info["child_id"],
                session_info["game_type"],
                summary.get("attention_type"),
                summary["final_score"],
                summary["total_accuracy"],
                summary["total_time"],
                summary["total_errors"],
                summary["levels_completed"],
                summary["avg_attention_score"],
                summary["max_attention_score"],
                summary["min_attention_score"],
                summary["attention_stability"],
                summary["avg_head_deviation"],
                summary["avg_blink_rate"],
        ...
        return {"success": False, "error": {"code": 500, "message": f"服务器内部错误：{str(e)}"}}, 500
...
def interrupt_session():
    ...
    return success_response({"session_id": session_id, "status": "interrupted"}, "会话已中断")
...
def get_training_history(child_id):
    ...
    return success_response({"total": total, "limit": limit, "offset": offset, "records": history}, "查询成功")
...
def get_training_detail(session_id):
    ...
    if vision_data:
        ...
        if head_yaws and head_pitchs:
            deviations = [math.sqrt(y**2 + p**2) for y, p in zip(head_yaws, head_pitchs, strict=False)]
        ...
        for row in records:
            try:
                ...
                if avg_score is not None:
                    trend_data[at]["records"].append({"date": date, "score": round(avg_score, 2)})
            except Exception:
                continue
            ...
            if at in trend_data:
                ...
                if records_list:
                    ...
                    if len(scores) >= 2:
                        ...
                        second_half = (
                            sum(scores[len(scores) // 2 :]) / (len(scores) - len(scores) // 2)
                            if len(scores) - len(scores) // 2 > 0
                            else 0
                        )
